...
def pairwise_oxy_angle(x: Tensor, y: Tensor, curv: float | Tensor = 1.0, eps: float = 1e-8):
    ...
    ### 각도를 확인해보기 왜 self 내적에서 각도가 pi/2가 나오지?
    ### 그리고 범위도 0에서 pi니까 맞게 코드 수정하기
...
def hyperboloid_to_klein(x: Tensor, curv: float | Tensor):
    return x/torch.sqrt(1/curv+torch.sum(x**2,dim=-1).unsqueeze(-1))

def klein_to_hyperboloid(k: Tensor, curv: float | Tensor):
    return k/torch.sqrt(curv*torch.square(1-torch.sum(k**2,dim=-1)).unsqueeze(-1))

def get_centroid(x: Tensor, curv: float | Tensor, eps: float = 1e-8):
    lorentz_factors = 1/torch.sqrt(torch.clamp(1-curv*torch.norm(hyperboloid_to_klein(x,curv)**2,dim=-1).unsqueeze(-1), min=eps))
    klein_centroid = torch.sum(lorentz_factors*hyperboloid_to_klein(x,curv)/torch.sum(lorentz_factors),dim=0).unsqueeze(0)
    hyperboloid_centroid = klein_to_hyperboloid(klein_centroid, curv)
    return pairwise_dist(hyperboloid_centroid, torch.zeros_like(hyperboloid_centroid))

def centroid_loss(x: Tensor, dist: float, curv: float | Tensor, eps: float = 1e-8):
    # return torch.abs(get_centroid(x, curv)-dist)
    return torch.norm(get_centroid(x, curv)-1/torch.sqrt(curv)*torch.acos(torch.clamp(curv*dist, min=-1+eps, max=1-eps)))
